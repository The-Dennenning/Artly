...
class Sorter : public Event{
	...
	public:
		Sorter(Bitmap *b, int start, int duration, int type, float* lower_bound, float* upper_bound) : 	Event(start, duration), _width(b->getSize(1)), _height(b->getSize(0)),
	...
	private:
		...
		void sort_line(Bitmap* b, int line_num);
...
void Sorter::Activate(Bitmap *b, int frame_num)
{
...
void Sorter::sort_line(Bitmap* _b, int line_num)
{
#ifdef DEBUG
